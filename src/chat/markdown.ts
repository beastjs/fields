...
import { diffBlock, fullFileDiff } from './diff';
...
export function renderChatMarkdown(source: string, original?: string) {
  ...
  renderer.code = token => diffBlock(token.text, original) ?? (original !== undefined && /\bfile=/.test(token.lang ?? '') ? fullFileDiff(token.text + '\n', original) : code(token));
