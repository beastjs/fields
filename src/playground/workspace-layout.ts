...
export const paneDefinitions = [
  { id: 'files', label: 'Files' }, { id: 'editor', label: 'Editor' },
  { id: 'preview', label: 'Preview' }, { id: 'output', label: 'Output' },
...
export class WorkspaceLayout {
  readonly panels = {
    files: ref<PanelImperativeHandle>(), editor: ref<PanelImperativeHandle>(),
    preview: ref<PanelImperativeHandle>(), output: ref<PanelImperativeHandle>(), chat: ref<PanelImperativeHandle>(),
  };
  readonly groups = { workspace: ref<GroupImperativeHandle>(), dock: ref<GroupImperativeHandle>(), main: ref<GroupImperativeHandle>() };
  ...
  sync = (sizes: Layout) => {
    const round = (keys: string[]) => {
      ...
      values[values.length - 1] = Math.round((100 - values.slice(0, -1).reduce((sum, value) => sum + value, 0)) * 100) / 100;
      return values;
    };
    ...
    for (const { id } of paneDefinitions) {
      const key = 'view-' + id;
      if (key in sizes && next[id] !== (sizes[key] > 0)) { next[id] = sizes[key] > 0; changed = true; }
    }
    if (changed) { this.visible = next; for (const listener of this.listeners) listener(); }
  };
  /** Persist settled changes, never every pixel of a pointer drag. */
  commit = () => {
    const defaults = this.resolve();
    this.persist?.({ dock: sameSizes(this.geometry.dock, defaults.dock) ? null : this.geometry.dock,
  ...
  private resolve(query?: PanelQuery) {
    return { dock: query?.dock && query.dock[1] >= 35 ? query.dock : [this.narrow ? 0 : 14, this.narrow ? 100 : 86, 0],
      split: query?.split ?? [50, 50], rows: query?.rows && query.rows[0] >= 30 ? query.rows : [76, 24] };
  }
  private layouts(geometry: { dock: number[]; split: number[]; rows: number[] }) {
    return { dock: { 'view-files': geometry.dock[0], center: geometry.dock[1], 'view-chat': geometry.dock[2] },
  ...
  restore = (query: PanelQuery) => {
    ...
    if (sameSizes(next.dock, this.geometry.dock) && sameSizes(next.split, this.geometry.split) && sameSizes(next.rows, this.geometry.rows)) return;
  ...
  collapse = (id: PaneId) => { if (this.canCollapse(id)) this.panels[id].current?.collapse(); };
