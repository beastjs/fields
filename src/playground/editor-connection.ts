...
export function connectEditor(parent: HTMLElement, session: PlaygroundSession) {
  ...
  const editor = new ProjectEditor(parent, session.updateSource, session.run, previous.keymap);
  ...
  const unsubscribe = session.subscribe(() => {
    ...
    queueMicrotask(() => {
      ...
      if (next.activeFile !== previous.activeFile) editor.open(next.activeFile, next.project.files[next.activeFile]);
