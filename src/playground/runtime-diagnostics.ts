...
export class RuntimeSourceMapper {
  ...
  private resolve(position: RuntimePosition, raw: string): RuntimeStackFrame {
    const moduleId = this.urls.get(position.url);
    ...
    try {
      if (!this.maps.has(module.id)) this.maps.set(module.id, new TraceMap(module.sourceMap));
      const map = this.maps.get(module.id);
      if (!map) return frame;
